...
> ANSWER: the EPSG code for the counties dataset is 4269. It is also a geographic coordinate reference system. "NAD 83" or "North American Datum of 1983" is what this CRS is associated to. 
> 
...
stnrdDF <- read.csv("./Data/Raw/NWIS_SiteInfo_NE_RAW.csv")
...
>ANSWER: The only new field to appear was geometry, and two disappeared: latitude and longitude (compiled into the one geometry field).
>
...
NE_Gage <- ggplot() +
  geom_sf(data = Counties_sf, color= 'red', size= 2) +
  geom_sf(data = NE_sf) +
...
GageHeightRaw <- read.csv("./Data/Raw/NWIS_SiteFlowData_NE_RAW.csv") %>%
...
#15. Show the column names
colnames(GageHeightRaw)

#16. Join the flow data to our NWIS gage location spatial dataframe
GHCounties_df_join <- merge(x = NE_sf,
                            y= GageHeightRaw,
                            by.x = "site_no",
                            by.y = "site_no")

#17. Show the column names of the joined dataset
colnames(GHCounties_df_join)

#18. Show the dimensions of this joined dataset
dim(GHCounties_df_join)
...
MapGH <- ggplot() + 
  ...
  geom_sf(data = GHCounties_df_join, aes(fill= meanGH, color= meanGH))
print(MapGH)
...
DougCounty <- Counties_sf %>%
  filter(NAME %in% c ("Douglas"))

#23 Select gages within the selected county
GagesDougCounty <- GHCounties_df_join %>%
  select(station_nm.y)

#24 Plot
DougGHPlot <- ggplot() +
  geom_sf(data = Counties_sf) +
  geom_sf(data= DougCounty, aes(color= NAME)) +
  geom_sf(data = GagesDougCounty, color= 'black')
print(DougGHPlot)
